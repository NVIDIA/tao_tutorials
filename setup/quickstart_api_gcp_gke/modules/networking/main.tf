...
resource "google_compute_address" "nat" {
  count  = length([for subnet in var.subnets : subnet if subnet.private]) > 0 ? 1 : 0
...
resource "google_compute_router_nat" "this" {
  ...
  nat_ips                            = [google_compute_address.nat[count.index].id]
  dynamic "subnetwork" {
...
resource "google_compute_firewall" "this" {
  ...
  source_ranges = each.value["nat_source"] ? concat(each.value["source_ranges"], [for nat_address in google_compute_address.nat.*.address : format("%s/32", nat_address)]) : each.value["source_ranges"]
