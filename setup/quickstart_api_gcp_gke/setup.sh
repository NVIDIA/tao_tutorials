...
function prepare_backend_config() {
  yq eval '.backend' deploy.yml -o=json | jq --arg key "$(backend_key)" '. += { prefix: ($key + "") }'
}
...
function download_cluster_credentials() {
  _project_id=$(yq eval '.provider.credentials' deploy.yml | jq -r '.project_id')
  _account=$(yq eval '.provider.credentials' deploy.yml | jq -r '.client_email')
  _region=$(yq eval '.spec.region' deploy.yml)
  _zone=$(yq eval '.spec.zone' deploy.yml)
  if [[ "$(gcloud config configurations list --format json | jq --arg name "${_project_id}" '. | map(.name) | map(select(. == $name)) | length')" == "1" ]]; then
    gcloud config configurations activate "${_project_id}"
  else
    gcloud config configurations create "${_project_id}"
  fi
  gcloud config set project "${_project_id}"
  gcloud config set account "${_account}"
  if [[ "$(gcloud auth list --format json | jq --arg account "${_account}" '. | map(.account) | map(select(. == $account)) | length')" == "0" ]]; then
    yq eval '.provider.credentials' deploy.yml > /tmp/gcp-credentials.json
  ...
  gcloud container clusters get-credentials "$(name)" --zone "${_zone}"
}
...
function cleanup_cluster_credentials() {
  ...
  _region=$(yq eval '.spec.region' deploy.yml)
  _zone=$(yq eval '.spec.zone' deploy.yml)
  _kubectl_user="gke_${_project_id}_${_zone}_$(name)"
  _kubectl_cluster="gke_${_project_id}_${_zone}_$(name)"
  _kubectl_context="gke_${_project_id}_${_zone}_$(name)"
  ...
  if [[ "$(gcloud auth list --format json | jq --arg account "${_account}" '. | map(.account) | map(select(. == $account)) | length')" != "0" ]]; then
    gcloud auth revoke "${_account}" --quiet
  fi
}
